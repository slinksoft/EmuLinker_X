...
public class Trivia implements Runnable {
	...
	public Trivia(KailleraServerImpl server){
        ...
        try{
        	server.announce("<Trivia> " + "Loading SupraSlinkTrivia Questions...", false, null);
			...
			if(questions.size() == 0){
				...
				server.announce("<Trivia> " + "SupraSlinkTrivia will begin in 10s!", false, null);
			}
        }
        catch(Exception e){
        	exitThread = true;
        	server.announce("<Trivia> " + "Error loading SupraSlinkTrivia Questions/Scores!", false, null);
        }
	}
		...
		for(int i = 0; i < scores.size(); i++){
			if(scores.get(i).getIP().equals(ip)){
				...
				if(s == 25){
					...
					server.announce("<Trivia> " + nick + ", you're a God at SupraSlinkTrivia!", false, null);
				}
				return;
			}
		}
	...
	public void saveScores(boolean display){
		try{
	        ...
	        if(display)
	        	server.announce("<Trivia> " + "SupraSlinkTrivia Scores were Saved Successfully!", false, null);
		}
        catch(Exception e){
        	server.announce("<Trivia> " + "Error Saving SupraSlinkTrivia Scores!", false, null);
        }
	}
}