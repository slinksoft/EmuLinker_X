...
public final class KailleraUserImpl implements KailleraUser, Executable
{
	...
	private static final String				EMULINKER_CLIENT_NAME 	= "Emulinker X Admin Client";
	...
	public synchronized void quitGame() throws DropGameException, QuitGameException, CloseGameException
	{
		...
		{
			// First set STATUS_IDLE and then call game.drop(), otherwise if someone quits the game without manually dropping - game status will not change to STATUS_WAITING
			setStatus(KailleraUser.STATUS_IDLE);
			game.drop(this, playerNumber);
		}
