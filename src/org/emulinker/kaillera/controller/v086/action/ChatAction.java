		...
		if(userN.getAccess() < AccessManager.ACCESS_ELEVATED){
			...
			if(doCommand){
				...
				if(((Chat) message).getMessage().equals("/alivecheck")){
					try {clientHandler.send(new InformationMessage(clientHandler.getNextMessageNumber(), "server", ":ALIVECHECK=Emulinker X Alive Check: You are still logged in."));} catch(Exception e) {}
				}
