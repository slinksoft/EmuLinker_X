...
public class AdminCommandAction implements V086Action
{
	...
	public static final String			COMMAND_TEMPMODERATOR	= "/tempmod"; //$NON-NLS-1$
	...
	public boolean isValidCommand(String chat){
		...
		else if (chat.equals(COMMAND_VERSION))
		{
			return true;
		}
	...
	private void processTrivia(String message, KailleraServerImpl server, KailleraUserImpl admin, V086Controller.V086ClientHandler clientHandler) throws ActionException, MessageFormatException
	{
		if(message.equals("/triviareset")){
			...
			server.announce("<Trivia> " + "SupraSlinkTrivia has been reset!", false, null);
		...
		else if (message.equals("/triviaon"))
		{
			...
			server.announce("SupraSlinkTrivia has been started!", false, null);
			Trivia trivia = new Trivia(server);
			Thread triviaThread = new Thread(trivia);
			triviaThread.start();
			server.setTriviaThread(triviaThread);
		...
		else if (message.equals("/triviaoff"))
		{
			...
			server.announce("SupraSlinkTrivia has been stopped!", false, null);
			...
			server.announce("<Trivia> " + "SupraSlinkTrivia will be paused after this question!", false, null);
		}
			...
			server.announce("<Trivia> " + "SupraSlinkTrivia has been resumed!", false, null);			
		}
	...
	private void processAnnounce(String message, KailleraServerImpl server, KailleraUserImpl admin, V086Controller.V086ClientHandler clientHandler) throws ActionException, MessageFormatException
	{
		...
		if(announcement.startsWith(":"))
			announcement = announcement.substring(1);	// this protects against people screwing up the emulinker supraslinkclient
	...
	private void processClear(String message, KailleraServerImpl server, KailleraUserImpl admin, V086Controller.V086ClientHandler clientHandler) throws ActionException, MessageFormatException
	{
		...
		if(admin.getAccess() == AccessManager.ACCESS_OWNER && server.getAccessManager().clearTemp(inetAddr, true) || admin.getAccess() == AccessManager.ACCESS_COOWNER && server.getAccessManager().clearTemp(inetAddr, true) || admin.getAccess() == AccessManager.ACCESS_SUPERADMIN && server.getAccessManager().clearTemp(inetAddr, true) || admin.getAccess() == AccessManager.ACCESS_ADMIN && server.getAccessManager().clearTemp(inetAddr, false))
